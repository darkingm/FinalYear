  ...
  const handlePay = async () => {
    ...
    try {
      ...
      const poll = async (): Promise<void> => {
        ...
        try {
          const statusRes = activeSession
            ? await getPaymentSessionStatus({
              sessionId: activeSession.session_id,
              nonce: activeSession.nonce,
            })
            : await paymentClient.get(`/api/payments/crypto/status/${orderId}`);
          const orderStatus = statusRes?.status?.status || statusRes?.status;
          const confs = Number(statusRes?.status?.confirmations ?? 0);
