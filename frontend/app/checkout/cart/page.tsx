...
function Steps({ current, t }: { current: number; t: (key: string) => string }) {
  const steps = [t('checkoutCart.step1'), t('checkoutCart.step2'), t('checkoutCart.step3')];
  return (
...
export default function CartCheckoutPage() {
  ...
  const [payStep, setPayStep] = useState<'idle' | 'approve' | 'sending' | 'confirming' | 'done'>('idle');
  ...
  const handleGetQuote = async () => {
    if (!isConnected || !address) { toast.error(t('checkoutCart.connectWalletFirst')); return; }
    setQuote(null);
    setQuoteError(null);
    ...
    try {
      ...
      if (orderIdsToQuote.length === 0) {
        toast.loading(t('checkoutCart.creatingOrders'), { id: 'create-orders' });
        ...
        if (!resCreate.data.success || !resCreate.data.orders?.length) {
          throw new Error(resCreate.data.message || t('checkoutCart.cannotCreateOrders'));
        }

        orderIdsToQuote = resCreate.data.orders.map((o: any) => o.order_id);
        setCreatedOrderIds(orderIdsToQuote);
        toast.success(t('checkoutCart.ordersCreatedToast', { count: orderIdsToQuote.length }), { id: 'create-orders' });
        clearCart(); // Cart items are now locked as DB orders
      }

      // Step B: Get batch crypto quote
      toast.loading(t('checkoutCart.gettingQuote'), { id: 'quote' });
      ...
      setQuote(q);
      setStep(3);
      toast.success(t('checkoutCart.quoteReady'), { id: 'quote' });
      ...
      if (chainId !== q.chain_id) {
        try {
          ...
          toast.info(t('checkoutCart.switchChainHint', { chain: chainName }));
        }
      }
    } catch (e: any) {
      const msg = e.response?.data?.message || e.message || t('checkoutCart.quoteFailed');
      setQuoteError(msg);
  ...
  const handleSwitchChain = async () => {
    ...
    catch { toast.error(t('checkoutCart.switchChainFailed')); }
  };

  const handleApprove = async () => {
    if (!quote || !address) return;
    if (nativeBalance && Number(nativeBalance.formatted) < 0.001) {
      toast.error(t('checkoutCart.gasError'));
      return;
    }
    setPayStep('approve');
    try {
      ...
      toast.loading(t('checkoutCart.waitingApprove'), { id: 'approve' });
      await new Promise(r => setTimeout(r, 4000));
      await refetchAllowance();
      toast.success(t('checkoutCart.approveSuccess'), { id: 'approve' });
    } catch (e: any) {
      const msg = e.shortMessage || e.message || '';
      if (e.code === 4001 || msg.includes('rejected')) toast.info(t('checkoutCart.approveCancelled'));
      else toast.error(`${t('checkoutCart.approveFailed')}: ${msg}`);
    } finally {
      setPayStep('idle');
    }
  };
  ...
  const handlePay = async () => {
    if (!quote || !walletClient || !address) { toast.error(t('checkoutCart.connectWallet')); return; }
    if (isWrongChain) { await handleSwitchChain(); return; }
    if (timeLeft === 0) { toast.error(t('checkoutCart.quoteExpired')); setQuote(null); setStep(2); return; }
    if (nativeBalance && Number(nativeBalance.formatted) < 0.001) {
      toast.error(t('checkoutCart.gasError'));
      return;
    }
    ...
    try {
      ...
      toast.loading(t('checkoutCart.waitingConfirm'), { id: 'tx' });
      ...
      setSubmitting(false);
      toast.success(t('checkoutCart.txSuccess'), { id: 'tx' });
      setTxHash(hash);

      const maxWaitMs = 120_000;
      const pollStart = Date.now();
      let pollCount = 0;
      const firstOrderId = quote.order_ids[0];

      const poll = async (): Promise<void> => {
        if (Date.now() - pollStart > maxWaitMs) {
          setPayStep('done');
          setConfirmed(true);
          toast.info(t('checkoutCart.stillConfirming'), { duration: 8000 });
          return;
        }
        try {
          ...
          if (orderStatus === 'ONCHAIN_CONFIRMED' || orderStatus === 'PAID') {
            setPayStep('done');
            setConfirmed(true);
            toast.success(t('checkoutCart.onchainConfirmed'), { duration: 6000 });
            let countdown = 5;
            setRedirectIn(countdown);
            const iv = setInterval(() => {
              countdown -= 1;
              setRedirectIn(countdown);
              if (countdown <= 0) { clearInterval(iv); router.push('/orders'); }
            }, 1000);
            return;
          }
        } catch { /* keep polling */ }
      ...
      const msg = e.shortMessage || e.message || t('checkoutCart.txFailed');
      if (e.code === 4001 || msg.includes('rejected')) toast.info(t('checkoutCart.txCancelled'));
  ...
  if (authLoading || loading) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-sm text-muted-foreground">{t('checkoutCart.loading')}</p>
      </div>
    );
  }
      ...
      <main className="flex-1 py-8 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-xl font-black">{t('checkoutCart.title')}</h1>
              <p className="text-xs text-muted-foreground font-mono">{t('checkoutCart.itemCount', { count: orderCount })}</p>
            </div>
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-bold text-sm">
                    {createdOrderIds.length > 0
                      ? t('checkoutCart.ordersCreated', { count: createdOrderIds.length })
                      : t('checkoutCart.itemsInCart', { count: items.length })}
                  </h3>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {getTotalItems()} items · {selectedToken} {t('checkoutCart.on')} {CHAIN_META[selectedNet]?.name || 'Hardhat'}
                  </p>
                </div>
                ...
                <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    >
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">{t('checkoutCart.cryptoBatchDesc')}</p>
                    </button>
                    <button
                      disabled
                    ...
                    >
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">{t('checkoutCart.fiatNotSupported')}</p>
                    </button>
                  </div>
                  ...
                  {displayItems.length > 0 && (
                    <div className="mt-4 space-y-2 border-t border-border pt-4">
                      <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">{t('checkoutCart.productsInCart')}</p>
              ...
              {step === 2 && (
                <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <h2 className="font-bold flex items-center gap-2 text-sm">
                    ...
                    {t('checkoutCart.chooseNetToken')}
                    <button onClick={() => setStep(1)} className="ml-auto text-xs text-muted-foreground hover:text-foreground">
                      {t('checkoutCart.goBack')}
                    </button>
                  </h2>

                  {/* Network selector */}
                  <div>
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">{t('checkoutCart.blockchain')}</p>
                  ...
                  {/* Token selector */}
                  <div>
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">{t('checkoutCart.paymentToken')}</p>
                  ...
                  <div className="p-4 bg-background border border-border rounded-xl flex items-center justify-between">
                    <div>
                      <p className="text-xs text-muted-foreground mb-1">{t('checkoutCart.estimated')}</p>
                      <p className="text-xl font-black font-mono">
                        {estimatedCrypto} <span className="text-sm text-[#8247e5]">{selectedToken}</span>
                      </p>
                      <p className="text-xs text-muted-foreground mt-0.5">≈ {totalUSDFormatted}</p>
                    </div>
                  </div>
                    ...
                    {!isConnected ? (
                      <ConnectButton.Custom>
                        {({ openConnectModal }) => (
                          ...
                          >
                            <Wallet className="w-4 h-4" /> {t('checkoutCart.connectMetamask')}
                          </button>
                        )}
                      </ConnectButton.Custom>
                    ) : (
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-xs text-emerald-400 font-semibold">{t('checkoutCart.connected')}</p>
                  ...
                  >
                    {quoteLoading
                      ? <><Loader2 className="w-5 h-5 animate-spin" />{t('checkoutCart.creatingAndQuoting')}</>
                      : <><Shield className="w-5 h-5" />{t('checkoutCart.confirmAndQuote')}</>}
                  </button>
                ...
                <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className="flex items-center justify-between">
                    <h2 className="font-bold flex items-center gap-2 text-sm">
                      ...
                      {t('checkoutCart.paymentTitle')} – {quoteNet?.name || `Chain ${quote.chain_id}`}
                    </h2>
                  ...
                  <div className="p-4 bg-background border border-border rounded-xl flex items-end justify-between">
                    <div>
                      <p className="text-xs text-muted-foreground mb-1">
                        {t('checkoutCart.batchPayment')} ({quote.order_ids.length})
                      </p>
                    ...
                    <div className="text-right text-xs text-muted-foreground">
                      <p>≈ {totalUSDFormatted}</p>
                      {quote.token_price > 0 && <p>1 {selectedToken} = {formatUSD(quote.token_price)}</p>}
                    </div>
                  </div>

                  {/* Confirming on-chain state */}
                  {payStep === 'confirming' && txHash && !confirmed ? (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                      className="text-center space-y-4 py-4">
                      <div className="w-20 h-20 rounded-full bg-purple-500/10 border-2 border-purple-500/30 flex items-center justify-center mx-auto">
                        <Loader2 className="w-10 h-10 text-purple-400 animate-spin" />
                      </div>
                      <h3 className="text-xl font-black text-purple-400">{t('checkoutCart.confirmingOnchain')}</h3>
                      <p className="text-sm text-muted-foreground">
                        {confirmCount > 0 ? `${confirmCount} confirmations...` : t('checkoutCart.waitingConfirm')}
                      </p>
                      <div className="bg-background border border-border rounded-xl p-3 text-left space-y-1">
                        <p className="text-[10px] text-muted-foreground font-medium uppercase tracking-wider">{t('checkoutCart.txHash')}</p>
                    ...
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                      ...
                      <h3 className="text-2xl font-black text-emerald-400">{t('checkoutCart.paymentSuccess')}</h3>
                      ...
                      {redirectIn !== null && redirectIn > 0 && (
                        <p className="text-xs text-muted-foreground animate-pulse">
                          {t('checkoutCart.redirecting', { count: redirectIn })}
                        </p>
                      )}
                    </motion.div>
                  ) : isWrongChain ? (
                    ...
                    >
                      ...
                      {t('checkoutCart.switchChain')} {quoteNet?.name || `Chain ${quote.chain_id}`}
                    </button>
                  ) : needsApprove ? (
                    <div className="p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl space-y-2">
                      <p className="text-sm font-bold text-amber-300">{t('checkoutCart.step1Approve')}</p>
                      <p className="text-xs text-amber-400/80">
                        {t('checkoutCart.approveDesc', { amount: quote.amount_token_total.toFixed(4), token: selectedToken })}
                      </p>
                      <button
                      ...
                      >
                        {payStep === 'approve'
                          ? <><Loader2 className="w-4 h-4 animate-spin" />{t('checkoutCart.approving')}</>
                    ...
                    >
                      {submitting
                        ? <><Loader2 className="w-5 h-5 animate-spin" />{t('checkoutCart.loading')}</>
                        : <><Wallet className="w-5 h-5" />{t('checkoutCart.signPay')}</>}
                    </button>
                  )}
                </motion.div>
              )}
            ...
            <div className="hidden lg:block">
              <div className="bg-card border border-border p-6 rounded-3xl sticky top-24 space-y-4">
                <h4 className="font-bold flex items-center gap-2">
                  ...
                  {t('checkoutCart.protection') || '100% Protection'}
                </h4>
                <p className="text-sm text-muted-foreground">
                  {t('checkoutCart.protectionDesc') || 'All payments go through Smart Contract Escrow.'}
                </p>
                <div className="space-y-2">
                  {[
                    { label: t('checkoutCart.batchPayment'), color: 'text-purple-400' },
                    { label: t('checkoutCart.autoSplit') || 'Auto-split for multiple sellers', color: 'text-emerald-400' },
                    { label: t('checkoutCart.refundDispute') || 'Refund if disputed', color: 'text-blue-400' },
                  ].map(({ label, color }) => (
